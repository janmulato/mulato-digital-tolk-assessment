...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $data['certified'] = $this->_getJobForCertified($data['job_for']);
            $data['job_type'] = $this->_getJobType($consumer_type);
            $data['gender'] = $this->_getJobGender($data['job_for']);
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $duration = $this->_convertToHoursMins($job->duration);
        $jobId = $job->id;
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $user_tags = $this->_getUserTagsStringFromArray($users);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data)
    {
        ...
        $changeTranslator = $this->_changeTranslator($current_translator, $data, $job);
        if ($changeTranslator['translatorChanged']) {
            $log_data[] = $changeTranslator['log_data'];
        }

        $changeDue = $this->_changeDue($job->due, $data['due']);
        ...
        $changeLang = $this->_changeLang($job->from_language_id, $data['from_language_id']);
        if ($changeLang['langChanged']) {
            $old_lang = $job->from_language_id;
            $job->from_language_id = $data['from_language_id'];
            $log_data[] = $changeLang['log_data'];
        }

        $changeStatus = $this->_changeStatus($job, $data, $changeTranslator['translatorChanged']);
        ...
        if ($job->due <= Carbon::now()) {
            ...
            if ($changeLang['langChanged']) $this->sendChangedLangNotification($job, $old_lang);
            return ['Updated'];
        }
    }
     ...
     * @return array
     */
    private function _changeStatus($job, $data, $changedTranslator)
    {
        ...
        if ($old_status != $data['status']) {
            switch ($job->status) {
                case 'timedout':
                    $statusChanged = $this->_changeTimedoutStatus($job, $data, $changedTranslator);
                    break;
                case 'completed':
                    $statusChanged = $this->_changeCompletedStatus($job, $data);
                    break;
                case 'started':
                    $statusChanged = $this->_changeStartedStatus($job, $data);
                    break;
                case 'pending':
                    $statusChanged = $this->_changePendingStatus($job, $data, $changedTranslator);
                    break;
                case 'withdrawafter24':
                    $statusChanged = $this->_changeWithdrawafter24Status($job, $data);
                    break;
                case 'assigned':
                    $statusChanged = $this->_changeAssignedStatus($job, $data);
                    break;
                default:
                    $statusChanged = false;
                    break;
     ...
     * @return bool
     */
    private function _changeTimedoutStatus($job, $data, $changedTranslator)
    {
        $user = $job->user()->first();
        if (!empty($job->user_email)) {
            $email = $job->user_email;
     ...
     * @return bool
     */
    private function _changeCompletedStatus($job, $data)
    {
     ...
     * @return bool
     */
    private function _changeStartedStatus($job, $data)
    {
     ...
     * @return bool
     */
    private function _changePendingStatus($job, $data, $changedTranslator)
    {
        if (($data['admin_comments'] ?? '') == '') return false;
        $job->admin_comments = $data['admin_comments'];
        $user = $job->user()->first();
     ...
     * @return bool
     */
    private function _changeWithdrawafter24Status($job, $data)
    {
        if (($data['admin_comments'] ?? '') == '') return false;
     ...
     * @return bool
     */
    private function _changeAssignedStatus($job, $data)
    {
        if (($data['admin_comments'] ?? '') == '') return false;
     ...
     * @return array
     */
    private function _changeTranslator($current_translator, $data, $job)
    {
        $log_data = [];
        $new_translator = null;
        ...
        if ((isset($data['translator']) && $data['translator'] != 0)) {
            ...
            } else if (is_null($current_translator) && (isset($data['translator']) && $data['translator'] != 0)) {
     ...
     * @return array
     */
    private function _changeDue($old_due, $new_due)
    {
        $dateChanged = false;
        if ($old_due != $new_due) {
            $log_data = [
     ...
     * @return array
     */
    private function _changeLang($old_lang, $new_lang)
    {
        $langeChanged = false;
        if ($old_lang != $new_lang) {
            $log_data[] = [
                'old_lang' => TeHelper::fetchLanguageFromJobId($old_lang),
                'new_lang' => TeHelper::fetchLanguageFromJobId($new_lang)
            ];
            $langChanged = true;

            return ['langChanged' => $langeChanged, 'log_data' => $log_data];
        }

        return ['langChanged' => $langeChanged];
    }
     ...
     * @param $duration
     */
    private function _sendNotificationChangePending($user, $job, $language, $due, $duration)
    {
        $data = array();
        $data['notification_type'] = 'session_start_remind';
        if ($job->customer_physical_type == 'yes') {
     ...
     * @return string
     */
    private function _getUserTagsStringFromArray($users)
    {
        $user_tags = "[";
        $first = true;
        foreach ($users as $oneUser) {
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            $allJobs = $this->getAllJobsForAdmin($consumer_type, $data);
        }
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
     ...
     * @return string         
     */
    private function _convertToHoursMins($time, $format = '%02dh %02dmin')
    {
        if ($time < 60) {
            return $time . 'min';
        } else if ($time == 60) {
    ...
    /**
     * Check if the array contains the following condition and return the certified
     * @param array $data
     * @return string
     */
    private function _getJobForCertified($data = [])
    {
        $certified = '';
    ...
    /**
     * Get job type based on consumer type
     *
     * @param string $consumer_type
     * @return string
     */
    private function _getJobType($consumer_type = '')
    {
        $jobtype = '';
    ...
    /**
     * Get job gender based on the job for array
     *
     * @param array $data
     * @return string
     */
    private function _getJobGender($data = [])
    {
        $gender = '';
